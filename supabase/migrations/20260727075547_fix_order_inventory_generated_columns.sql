-- New orders must never provide values for GENERATED ALWAYS summary columns.
--
-- `public.orders.item_count` and `public.orders.item_total_qty` are stored
-- generated columns. Expanding an `orders%ROWTYPE` into INSERT supplied NULL
-- for both columns, which PostgreSQL correctly rejected before any order or
-- inventory reservation could be committed. Keep the read model intact and
-- insert only writable business columns.
