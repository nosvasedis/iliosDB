...
import { ProductionBatch, Order, Product, Mold, OrderStatus } from '../types';
...
export default function ProductionMoldRequirementsModal({ isOpen, onClose, orders, batches, products, molds }: Props) {
    ...
    // Identify orders that actually have batches in production (exclude delivered/cancelled)
    const relevantOrders = useMemo(() => {
        const orderIdsWithBatches = new Set(batches.map(b => b.order_id));

        return orders
            .filter(o => orderIdsWithBatches.has(o.id) && o.status !== OrderStatus.Delivered && o.status !== OrderStatus.Cancelled)
            .filter(o =>
    ...
    // Aggregate mold requirements for selected orders
                    ...
                    <div className="w-[350px] bg-slate-50 border-r border-slate-200 flex flex-col shrink-0">
                        <div className="p-4 border-b border-slate-200 space-y-3">
                            <div className="relative">
                                ...
                                <input
                                    type="text"
                                    placeholder="Αναζήτηση εντολής ή πελάτη..."
                                    value={searchTerm}
