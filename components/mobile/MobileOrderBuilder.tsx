...
const STONE_TEXT_COLORS: Record<string, string> = {
    ...
    'PCO': 'text-emerald-400', 'MCO': 'text-purple-500', 'PAX': 'text-green-500', 'MAX': 'text-blue-600', 
    'KAX': 'text-red-600', 'AI': 'text-slate-500', 'AP': 'text-cyan-500', 'AM': 'text-teal-600', 
    'LR': 'text-indigo-600', 'BST': 'text-sky-400', 'MP': 'text-blue-400', 'LE': 'text-slate-300', 
    'PR': 'text-green-400', 'KO': 'text-red-500', 'MV': 'text-purple-400', 'RZ': 'text-pink-500', 
    'AK': 'text-cyan-300', 'XAL': 'text-stone-400'
};
...
export default function MobileOrderBuilder({ onBack, initialOrder, products }: Props) {
    ...
    const [scanInput, setScanInput] = useState(''); // Separate scan input for Smart Entry
    ...
    // Calculate Totals
    const subtotal = selectedItems.reduce((sum, i) => sum + (i.price_at_order * i.quantity), 0);
    ...
    const handleSaveOrder = async () => {
        ...
        if (selectedItems.length === 0) { showToast("Η παραγγελία είναι κενή.", 'error'); return; }
                ...
                <div className="lg:col-span-3 bg-white rounded-3xl border border-slate-100 p-6 shadow-sm overflow-y-auto custom-scrollbar h-full">
                    ...
                    <div className="space-y-4">
                        <div className="relative">
                            ...
                            {showCustomerResults && customerName && !selectedCustomerId && filteredCustomers.length > 0 && (
