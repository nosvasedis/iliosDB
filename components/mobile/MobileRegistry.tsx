...
export default function MobileRegistry({ products, onProductSelect }: Props) {
    ...
    const filteredProducts = useMemo(() => {
        return products.filter(p => {
            ...
            return matchesSearch && matchesCategory && !p.is_component; // Hide components in catalog view
        }).slice(0, 50); // Limit for performance
    }, [products, search, selectedCategory]);

    return (
        <div className="p-4 h-full flex flex-col">
            <h1 className="text-2xl font-black text-slate-900 mb-4">Μητρώο</h1>
                    ...
                    {filteredProducts.map(p => (
                        ...
                        >
                            ...
                            <div className="mt-auto">
                                ...
                                <div className="mt-1 font-bold text-slate-900 text-xs">{formatCurrency(p.selling_price)}</div>
                            </div>
                        </div>
                    ))}
                </div>
