    ...
    return (
        <div className="fixed inset-0 z-[110] flex flex-col bg-slate-50 animate-in slide-in-from-bottom duration-300">
            ...
            <div className="flex-1 space-y-4 overflow-y-auto px-4 pb-28 pt-4 custom-scrollbar">
                ...
                {needsLoading && (
                    <div className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-white p-3 text-xs font-bold text-slate-500">
                ...
                <div ref={cartSectionRef} className="scroll-mt-24 rounded-2xl border border-slate-200 bg-white shadow-sm">
                    ...
                    <div className="space-y-2 p-3">
                        {filteredItems.map(({ item, index }) => {
                            ...
                            return (
                                <div key={`${item.id}-${index}`} className={`rounded-2xl border bg-white p-3 shadow-sm ${item.source_allocations?.some((allocation) => conflictingSourceIds.has(allocation.source_id)) ? 'border-red-400 ring-2 ring-red-100' : 'border-slate-100'}`}>
                                    <div className="flex items-start justify-between gap-2">
                                        <div className="flex min-w-0 gap-3">
                                                    ...
                                                    <div className="mt-2 flex flex-wrap gap-1">
                                                        {item.source_allocations.map((allocation) => (
                                                            <button
                                                                ...
                                                                className="inline-flex items-center gap-1 rounded-lg border border-indigo-100 bg-indigo-50 px-2 py-1 text-[9px] font-black text-indigo-700"
                                                            >
                                                                {allocation.customer} · {allocation.quantity} <X size={9} />
                                                            </button>
                                                        ))}
                                                    </div>
                                                )}
                                    ...
                                    <div className="mt-3 grid grid-cols-12 gap-2">
                                        <div className={showSizeInput ? 'col-span-5' : 'col-span-6'}>
                                            <label className={`${fieldLabel} mb-1 block`}>
                                                {allocatedQty > 0 ? `Έξτρα · ${allocatedQty} δεσμευμένα` : 'Ποσότητα'}
                                            </label>
