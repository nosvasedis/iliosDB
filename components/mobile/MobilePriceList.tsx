...
export default function MobilePriceList({ onPrint }: Props) {
    ...
    const displayedItems = useMemo(() => {
        ...
        if (mode === 'collections') {
            ...
            // Custom Mode: If search is active, show search results. If not, show the custom list.
            if (search) {
                const lower = search.toLowerCase();
    ...
    const toggleCustomItem = (sku: string) => {
        ...
        if (search) {
            showToast(customList.includes(sku) ? 'Αφαιρέθηκε.' : 'Προστέθηκε.', 'success');
        }
    };
    ...
    // --- PRINT GENERATION ---
    const generatePrintData = (): PriceListPrintData => {
        const itemsToPrint = mode === 'custom' 
            ? products?.filter(p => customList.includes(p.sku)) || []
            : displayedItems; // For collections, print what's currently filtered/selected
        ...
        if (mode === 'custom') title = 'Επιλεγμένα Είδη';
            ...
            title = collections?.find(c => c.id === selectedCollection)?.name || 'Συλλογή';
        }
    ...
    const handlePrint = () => {
        ...
        if (data.items.length === 0) {
            showToast("Δεν υπάρχουν είδη προς εκτύπωση.", "error");
            return;
        }
    ...
    return (
        <div className="p-4 h-full flex flex-col pb-24">
            <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={handlePrint} className="p-2 bg-slate-900 text-white rounded-xl shadow-md active:scale-95 transition-transform">
                    <Printer size={20}/>
                </button>
            </div>
            ...
            <div className="flex bg-slate-100 p-1 rounded-xl mb-4 shrink-0">
                <button 
                    onClick={() => { setMode('collections'); setSearch(''); }}
                    className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-2 ${mode === 'collections' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`}
                >
                    <FolderKanban size={14}/> Συλλογές
                </button>
                <button 
                    onClick={() => { setMode('custom'); setSearch(''); }}
                    className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-2 ${mode === 'custom' ? 'bg-white text-teal-600 shadow-sm' : 'text-slate-500'}`}
                >
                    <ListPlus size={14}/> Custom ({customList.length})
                </button>
            </div>
            ...
            <div className="space-y-4 flex-1 flex flex-col min-h-0">
                <div className="relative shrink-0">
                    ...
                    <input 
                        type="text" 
                        placeholder={mode === 'custom' ? "Αναζήτηση για προσθήκη..." : "Αναζήτηση..."}
                        value={search}
                    ...
                    />
                    {mode === 'custom' && !search && customList.length > 0 && (
                        <button onClick={clearCustomList} className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 bg-slate-100 rounded-lg text-slate-400 hover:text-red-500">
                            <Trash2 size={14}/>
                        </button>
                    )}
                </div>
                    ...
                    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                            ...
                            return (
                                <div key={p.sku} className={`p-4 flex justify-between items-center ${idx < displayedItems.length - 1 ? 'border-b border-slate-50' : ''}`}>
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        {mode === 'custom' && (
                                            <button 
                                                onClick={() => toggleCustomItem(p.sku)}
                                                className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${isSelected ? 'bg-red-50 text-red-500' : 'bg-teal-50 text-teal-600'}`}
                                            >
                        ...
                        {displayedItems.length === 0 && (
                            <div className="p-12 text-center text-slate-400 text-sm flex flex-col items-center">
                                <ListPlus size={32} className="mb-2 opacity-30"/>
                                {mode === 'custom' && !search ? 'Η λίστα είναι κενή.' : 'Κανένα αποτέλεσμα.'}
                            </div>
                        )}
                    </div>
                </div>
