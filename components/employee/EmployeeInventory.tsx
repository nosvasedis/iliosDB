...
const STONE_TEXT_COLORS: Record<string, string> = {
    ...
    'KAX': 'text-red-700', 'AI': 'text-slate-600', 'AP': 'text-cyan-600', 'AM': 'text-teal-700', 'AZM': 'text-teal-600', 'LR': 'text-indigo-700', 'SB': 'text-sky-500',
        ...
        try {
            ...
            if (activeVariant) {
                if (isCentral) {
                    ...
                    await supabase.from('product_variants').update({ stock_qty: newTotal, stock_by_size: newMap }).match({ product_sku: targetSku, suffix: targetSuffix });
                } else {
                ...
                if (isCentral) {
                    ...
                    await supabase.from('products').update({ stock_qty: Math.max(0, (activeMaster.stock_qty || 0) + delta), stock_by_size: newMap }).eq('sku', targetSku);
                } else if (isShowroom) {
