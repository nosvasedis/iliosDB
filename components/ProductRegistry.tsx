...
export default function ProductRegistry({ setPrintItems }: Props) {
    ...
    return (
        <div className="flex flex-col space-y-3">
            <div ref={headerRef} className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 shrink-0">
                <div className="bg-white p-3 px-4 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between gap-4">
                    <div>
                        <h1 className="text-xl font-bold text-[#060b00] tracking-tight flex items-center gap-2">
                            <div className="p-1 px-1.5 bg-emerald-100 text-emerald-700 rounded-lg">
                                <Database size={18} />
                            </div>
                            Μητρώο Κωδικών
                        </h1>
                        <p className="text-slate-500 mt-0.5 ml-8 text-[11px] leading-tight">
                ...
                <div className="flex items-center gap-2 self-stretch md:self-auto w-full md:w-auto">
                    <div className="flex bg-slate-100 p-1 rounded-lg">
                        <button onClick={() => setViewMode('grid')} className={`p-1.5 flex items-center justify-center rounded-md transition-all ${viewMode === 'grid' ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>
                            <LayoutGrid size={18} />
                        </button>
                        <button onClick={() => setViewMode('table')} className={`p-1.5 flex items-center justify-center rounded-md transition-all ${viewMode === 'table' ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>
                            <List size={18} />
                        </button>
                    </div>
                    <div className="w-px h-5 bg-slate-200 hidden md:block mx-0.5"></div>
                    <div className="flex bg-slate-100 p-1 rounded-lg">
                        <button onClick={() => setShowStxOnly(false)} className={`px-3 py-1.5 rounded-md font-bold text-[13px] transition-all flex items-center gap-1.5 ${!showStxOnly ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                            <Database size={14} /> <span className="hidden sm:inline">Προϊόντα</span>
                        </button>
                        <button onClick={() => setShowStxOnly(true)} className={`px-3 py-1.5 rounded-md font-bold text-[13px] transition-all flex items-center gap-1.5 ${showStxOnly ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                            <Puzzle size={14} /> <span className="hidden sm:inline">Στοιχεία</span>
                        </button>
                    </div>
                    <div className="flex gap-1.5">
                        <button onClick={() => setShowScanner(true)} className="flex items-center justify-center gap-1.5 bg-white border border-slate-200 text-slate-600 px-3 py-1.5 rounded-lg font-bold text-[13px] transition-all shadow-sm hover:bg-slate-50">
                            <Camera size={16} /> <span className="hidden sm:inline">Σάρωση</span>
                        </button>
                        <button onClick={() => setIsCreating(true)} className="flex items-center justify-center gap-1.5 bg-[#060b00] hover:bg-black text-white px-3 py-1.5 rounded-lg font-bold text-[13px] transition-all shadow-md hover:shadow-lg">
                            <PackagePlus size={16} /> <span className="whitespace-nowrap">Νέο</span>
                        </button>
                    </div>
                </div>
            </div>

            <div className="bg-white p-2.5 rounded-2xl shadow-sm border border-slate-100 flex flex-col md:flex-row gap-3 items-center shrink-0">
                <div className="relative group flex-1 w-full">
                    ...
                    <input type="text" placeholder="Αναζήτηση Κωδικού (π.χ. K14300) ή Κατηγορίας..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-9 pr-3 py-1.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none w-full bg-slate-50 focus:bg-white transition-all text-slate-900 text-sm font-medium" />
                </div>
                <div className="flex gap-2 w-full md:w-auto">
                    <button onClick={() => setShowFiltersSidebar(true)} className={`flex-1 md:flex-none relative px-4 py-1.5 rounded-lg font-bold text-[13px] transition-all border flex items-center justify-center gap-1.5 ${(filterCategory !== 'All' || filterGender !== 'All' || subFilters.stone !== 'all' || subFilters.plating !== 'all') ? 'bg-[#060b00] text-white shadow-md border-black' : 'bg-white text-slate-700 border-slate-200 hover:bg-slate-50'}`}>
                        <Filter size={14} />
                        <span>Φίλτρα</span>
                        {(filterCategory !== 'All' || filterGender !== 'All' || subFilters.stone !== 'all' || subFilters.plating !== 'all') && (
                            <span className="flex items-center justify-center w-3.5 h-3.5 ml-1 text-[9px] bg-emerald-500 text-white rounded-full">!</span>
                        )}
                    </button>
                </div>
            </div>

            <div ref={scrollContainerRef} className="overflow-y-auto custom-scrollbar pr-1 relative" style={{ maxHeight: 'calc(100dvh - 12rem)' }}>
                {viewMode === 'grid' ? (
                    /* ── GRID (PAGINATED) ── */
