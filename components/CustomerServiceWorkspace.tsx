...
function BulkConsignmentCreator({ customers, products, orders, existingOrderLineIds, warehouses, defaultSellerId, onClose, onSubmit, saving }: { customers: any[]; products: Product[]; orders: any[]; existingOrderLineIds: Set<string | null>; warehouses: any[]; defaultSellerId?: string; onClose: () => void; onSubmit: (groups: any[]) => Promise<void>; saving: boolean }) {
  ...
  const valid = rows.length > 0 && rows.every((row) => row.customerId && row.warehouseId
    && catalogSkus.has(row.sku)
    && row.quantity > 0 && row.unitPrice >= 0 && row.unitCost >= 0);
  ...
  return (
    <ModalShell title="Μαζικός Δημιουργός Παρακαταθηκών" subtitle="Πολλοί πελάτες, μία ελεγχόμενη και ατομική συνεδρία." onClose={onClose} wide>
      <div className="space-y-4">
        <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white">
          <table className="w-full min-w-[1120px] text-left text-xs">
            <thead className="bg-slate-100 text-[10px] font-black uppercase text-slate-500">
              <tr><th className="p-3">Πελάτης</th><th className="p-3">Έξυπνο SKU</th><th className="p-3">Ποσότητα</th><th className="p-3">Κόστος</th><th className="p-3">Τιμή</th><th className="p-3">Αποθήκη</th><th className="p-3">Επανέλεγχος</th><th className="w-12 p-3"></th></tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {rows.map((row) => (
                <tr key={row.id} className="align-top">
                  <td className="p-2"><select className={inputClass} value={row.customerId} onChange={(event) => update(row.id, { customerId: event.target.value })}><option value="">Επιλέξτε…</option>{customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.full_name}</option>)}</select></td>
                  <td className="min-w-[280px] p-2">
                    <SkuProductPicker
                      ...
                      catalogOnly
                    />
                    <div className="mt-1 text-[10px] font-bold text-emerald-700">Κόστος και τιμή ενημερώνονται από την επιλεγμένη παραλλαγή.</div>
                  </td>
                  <td className="p-2"><input type="number" min={1} className={inputClass} value={row.quantity} onChange={(event) => update(row.id, { quantity: Number(event.target.value) })} /></td>
                  <td className="p-2"><input aria-label="Κόστος ανά τεμάχιο" type="number" min={0} step="0.01" className={inputClass} value={row.unitCost} onChange={(event) => update(row.id, { unitCost: Number(event.target.value) })} /></td>
                  <td className="p-2"><input aria-label="Τιμή ανά τεμάχιο" type="number" min={0} step="0.01" className={inputClass} value={row.unitPrice} onChange={(event) => update(row.id, { unitPrice: Number(event.target.value) })} /></td>
                  <td className="p-2"><select className={inputClass} value={row.warehouseId} onChange={(event) => update(row.id, { warehouseId: event.target.value })}>{warehouses.map((warehouse) => <option key={warehouse.id} value={warehouse.id}>{warehouse.name}</option>)}</select></td>
                  <td className="p-2"><input type="date" className={inputClass} value={row.reviewDate} onChange={(event) => update(row.id, { reviewDate: event.target.value })} /></td>
                  <td className="p-2"><button type="button" aria-label="Αφαίρεση γραμμής" onClick={() => setRows((current) => current.filter((item) => item.id !== row.id))} className="rounded-lg p-2 text-slate-400 hover:bg-rose-50 hover:text-rose-600"><X size={16} /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        <div className="flex flex-col gap-3 rounded-2xl bg-slate-900 p-4 text-white sm:flex-row sm:items-center sm:justify-between">
          <button type="button" className="inline-flex items-center gap-2 text-sm font-black" onClick={() => setRows((current) => [...current, newConsignmentRow()])}><Plus size={17} /> Προσθήκη γραμμής</button>
          <div className="flex flex-wrap gap-5 text-xs"><span>{rows.reduce((sum, row) => sum + row.quantity, 0)} τεμάχια</span><span>Κόστος <strong>{formatGreekMoney(totalCost)}</strong></span><span>Αξία <strong>{formatGreekMoney(totalValue)}</strong></span></div>
        </div>
        <div className="flex justify-end gap-2"><button className={secondaryButton} onClick={onClose}>Ακύρωση</button><button className={primaryButton} disabled={!valid || saving} onClick={submit}>{saving && <Loader2 size={16} className="animate-spin" />} Δημιουργία όλων</button></div>
      </div>
    </ModalShell>
  );
}

function RepairIntakeCreator({ customers, products, orders, consignmentSales, previousRepairItem, defaultSellerId, onClose, onSubmit, saving }: { customers: any[]; products: Product[]; orders: any[]; consignmentSales: ConsignmentSaleOption[]; previousRepairItem?: RepairItem | null; defaultSellerId?: string; onClose: () => void; onSubmit: (input: any) => Promise<void>; saving: boolean }) {
  ...
  const customerConsignmentSales = useMemo(() => consignmentSales.filter((sale) => sale.customerId === customerId), [consignmentSales, customerId]);
  const purchasedSkus = useMemo(() => new Set(customerOrders.flatMap((order) => order.items?.map((item: any) => item.sku) || [])), [customerOrders]);
  const sortedProducts = useMemo(() => [...products].sort((a, b) => Number(purchasedSkus.has(b.sku)) - Number(purchasedSkus.has(a.sku)) || a.sku.localeCompare(b.sku)), [products, purchasedSkus]);
  const update = (id: string, patch: Partial<RepairDraftRow>) => setRows((current) => current.map((row) => row.id === id ? { ...row, ...patch } : row));
  const selectCatalogSku = (id: string, sku: string, variantSuffix: string | null) => update(id, {
    sku,
    variantSuffix: variantSuffix || '',
  });
  ...
  return (
    <ModalShell title="Παραλαβή Επισκευών" subtitle="Μία παραλαβή πελάτη με ξεχωριστή ταυτότητα και παρτίδα ανά φυσικό τεμάχιο." onClose={onClose} wide>
      <div className="space-y-4">
        {previousRepairItem && <div className="rounded-2xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-800"><strong>Συνδεδεμένη επανεπισκευή:</strong> το νέο δελτίο θα συνδεθεί με το κλεισμένο {previousRepairItem.code}, χωρίς να το ξανανοίξει.</div>}
        <div className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 md:grid-cols-2">
          <label><span className="mb-1 block text-xs font-bold text-slate-600">Πελάτης *</span><select className={inputClass} value={customerId} onChange={(event) => setCustomerId(event.target.value)} disabled={Boolean(previousRepairItem)}><option value="">Επιλέξτε πελάτη…</option>{customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.full_name}</option>)}</select></label>
          <label><span className="mb-1 block text-xs font-bold text-slate-600">Σημειώσεις παραλαβής</span><input className={inputClass} value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Προαιρετική κοινή σημείωση" /></label>
        </div>
        {rows.map((row, index) => {
          const selectedOrder = customerOrders.find((order) => order.id === row.orderId);
          return (
            <div key={row.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="mb-3 flex items-center justify-between"><h3 className="font-black text-slate-800">Τεμάχιο {index + 1}</h3><button className="rounded-lg p-2 text-slate-400 hover:bg-rose-50 hover:text-rose-600" onClick={() => setRows((current) => current.filter((item) => item.id !== row.id))} aria-label="Αφαίρεση τεμαχίου"><X size={16} /></button></div>
              <div className="grid gap-3 md:grid-cols-3">
                <label><span className="mb-1 block text-xs font-bold text-slate-600">Προέλευση *</span><select className={inputClass} value={row.originType} onChange={(event) => update(row.id, { originType: event.target.value as RepairOriginType, orderId: '', orderLineId: '', sourceConsignmentSettlementId: '' })}>{Object.entries(REPAIR_ORIGIN_LABELS).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label>
                {row.originType === 'recorded_sale' && (
                  <>
                    ...
                    <label><span className="mb-1 block text-xs font-bold text-slate-600">Πώληση Παρακαταθήκης</span><select className={inputClass} value={row.sourceConsignmentSettlementId} onChange={(event) => { const sale = customerConsignmentSales.find((entry) => entry.id === event.target.value); update(row.id, { sourceConsignmentSettlementId: event.target.value, orderId: '', orderLineId: '', sku: sale?.sku || row.sku, variantSuffix: sale?.variantSuffix || '', sizeInfo: sale?.sizeInfo || '' }); }}><option value="">Καμία πώληση Παρακαταθήκης</option>{customerConsignmentSales.map((sale) => <option key={sale.id} value={sale.id}>{sale.sku}{sale.variantSuffix} · {formatGreekDateOnly(sale.soldAt)}</option>)}</select></label>
                  </>
                )}
                <div className="md:col-span-2">
                  <span className="mb-1 block text-xs font-bold text-slate-600">Έξυπνο SKU</span>
                  <div className="flex items-center gap-2">
                    <SkuProductPicker
                      ...
                      catalogOnly
                    />
                    {row.sku && <button type="button" className={secondaryButton} onClick={() => update(row.id, { sku: '', variantSuffix: '', sizeInfo: '' })} aria-label="Αφαίρεση SKU"><X size={15} /></button>}
                  </div>
                  {row.sku && purchasedSkus.has(row.sku) && <div className="mt-1 text-[10px] font-black text-emerald-700">Υπάρχει στο ιστορικό αγορών του πελάτη.</div>}
                </div>
                {selectedOrder && <label><span className="mb-1 block text-xs font-bold text-slate-600">Γραμμή πώλησης</span><select className={inputClass} value={row.orderLineId} onChange={(event) => { const item = selectedOrder.items?.find((entry: any) => entry.line_id === event.target.value); update(row.id, { orderLineId: event.target.value, sku: item?.sku || row.sku, variantSuffix: item?.variant_suffix || '', sizeInfo: item?.size_info || '' }); }}><option value="">Επιλέξτε…</option>{selectedOrder.items?.map((item: any) => <option key={item.line_id} value={item.line_id}>{item.sku}{item.variant_suffix || ''}</option>)}</select></label>}
                <label className="md:col-span-2"><span className="mb-1 block text-xs font-bold text-slate-600">Περιγραφή βλάβης *</span><textarea className={`${inputClass} min-h-20`} value={row.description} onChange={(event) => update(row.id, { description: event.target.value })} placeholder="Τι χρειάζεται επισκευή;" /></label>
                <label><span className="mb-1 block text-xs font-bold text-slate-600">Κατάσταση παραλαβής</span><input className={inputClass} value={row.intakeCondition} onChange={(event) => update(row.id, { intakeCondition: event.target.value })} placeholder="Γρατζουνιές, φθορά…" /></label>
                <label><span className="mb-1 block text-xs font-bold text-slate-600">Παρελκόμενα</span><input className={inputClass} value={row.accessories} onChange={(event) => update(row.id, { accessories: event.target.value })} placeholder="Κουτί, αλυσίδα…" /></label>
              </div>
            </div>
          );
        })}
        {!previousRepairItem && <button className={secondaryButton} onClick={() => setRows((current) => [...current, newRepairRow()])}><Plus size={16} /> Προσθήκη τεμαχίου</button>}
        <div className="flex justify-end gap-2"><button className={secondaryButton} onClick={onClose}>Ακύρωση</button><button className={primaryButton} disabled={!valid || saving} onClick={submit}>{saving && <Loader2 size={16} className="animate-spin" />} Οριστικοποίηση παραλαβής</button></div>
      </div>
    </ModalShell>
  );
}
  ...
  return (
    <ModalShell title={title} onClose={onClose}>
      <div className="space-y-4">
        ...
        {operation.kind === 'quality' && !operation.passed && <label><span className="mb-1 block text-xs font-bold text-slate-600">Στάδιο επιστροφής</span><select className={inputClass} value={stage} onChange={(event) => setStage(event.target.value)}><option value="Waxing">Κέρωμα</option><option value="Casting">Χύτευση</option><option value="Setting">Καρφωτική</option><option value="Polishing">Λουστράρισμα</option><option value="Assembly">Συναρμολόγηση</option><option value="Labeling">Ετικετοποίηση</option></select></label>}
        {operation.kind === 'cost' && (
          <>
            <label><span className="mb-1 block text-xs font-bold text-slate-600">Τύπος κόστους</span><select className={inputClass} value={costType} onChange={(event) => { setCostType(event.target.value as typeof costType); setSku(''); setVariantSuffix(''); }}><option value="labor">Εργασία</option><option value="material">Υλικό</option><option value="component">Εξάρτημα</option><option value="external">Εξωτερική εργασία</option></select></label>
