        ...
        <div className="bg-white text-black font-sans w-[210mm] min-h-[297mm] p-6 mx-auto shadow-lg print:shadow-none print:p-6 page-break-after-always flex flex-col relative">
            ...
            <div className="flex justify-between items-end border-b-2 border-slate-900 pb-2 mb-3 shrink-0">
                ...
                <div className="text-right">
                    <h1 className="text-xl font-black text-slate-900 uppercase tracking-tight leading-none mb-0.5">Δελτιο Αποστολης</h1>
            ...
            <div className="flex gap-4 mb-3 shrink-0 bg-slate-50 rounded-lg border border-slate-200 p-2">
                ...
                <div className="flex flex-col justify-center items-end px-2 min-w-[120px]">
                    <span className="text-[9px] font-bold text-slate-500 uppercase tracking-wider">Αξια Αποστολης</span>
            ...
            <div className="flex gap-4 mb-3 text-[10px] text-slate-600 font-medium bg-amber-50 rounded-lg border border-amber-100 px-3 py-2">
                <span>Απεσταλη απο: <span className="font-bold text-slate-800">{shipment.shipped_by}</span></span>
                {shipment.notes && (
                    <>
                        <span className="text-slate-300">|</span>
            ...
            <main className="flex-1 min-h-0 relative">
                <div className="flex border-b-2 border-slate-800 pb-1 mb-1 text-[10px] font-black text-slate-700 uppercase tracking-wider">
                    ...
                    <div className="w-10 text-center">Ποσ.</div>
                    <div className="w-16 text-right">Τιμη</div>
                    <div className="w-16 text-right">Συνολο</div>
                </div>
                ...
                <div className="text-[12px] leading-snug">
                    {sortedShipmentItems.map((item, index) => {
                        ...
                        return (
                            <div key={index} className="flex items-center py-1.5 border-b border-slate-100 break-inside-avoid">
                                ...
                                <div className="flex-1 px-1 min-w-0">
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-[10px] text-slate-600 truncate max-w-[250px] font-medium">{description}</span>
                                    </div>
                                </div>
                                <div className="w-10 text-center font-bold text-slate-800 text-[12px]">{item.quantity}</div>
                                <div className="w-16 text-right text-slate-700 tabular-nums font-semibold text-[12px]">{item.price_at_order.toFixed(2).replace('.', ',')}</div>
                                <div className="w-16 text-right font-black text-slate-900 tabular-nums text-[12px]">{(item.price_at_order * item.quantity).toFixed(2).replace('.', ',')}</div>
                            </div>
                        );
                    })}
                </div>
