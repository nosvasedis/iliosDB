...
import { Trash2, Plus, Save, Globe, Phone, Mail, MapPin, Search, Edit, Package, X, Check, Link, ImageIcon, Box, ShoppingCart, TrendingUp, Clock, Calendar, CheckCircle, List, ArrowRight, FileText } from 'lucide-react';
...
export default function SuppliersPage() {
  ...
  const assignedMaterials = useMemo(() => {
      if (!materials || !selectedSupplier) return [];
      return materials
  ...
  const relatedOrders = useMemo(() => {
      if (!supplierOrders || !selectedSupplier) return [];
  ...
  const handleAddToOrder = (item: any, type: SupplierOrderType) => {
      ...
      const cost = type === 'Product' ? (item.supplier_cost || item.active_price || 0) : item.cost_per_unit;
        ...
        <div className="flex-1 bg-white rounded-3xl border border-slate-100 flex flex-col overflow-hidden shadow-sm relative">
            {selectedSupplier ? (
                    ...
                    <div className="flex-1 overflow-y-auto p-6 bg-slate-50/30 custom-scrollbar">
                        ...
                        {activeTab === 'products' && (
                            <div className="space-y-4">
                                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center gap-4">
                                    ...
                                    <input className="flex-1 outline-none text-sm font-medium" placeholder="Αναζήτηση κωδικού για σύνδεση..." value={productSearchTerm} onChange={e => setProductSearchTerm(e.target.value)} />
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    {assignedProducts.map(p => {
                                        const costToDisplay = p.production_type === ProductionType.Imported && p.supplier_cost ? p.supplier_cost : p.active_price;
                                        return (
                                            <div key={p.sku} className="bg-white p-3 rounded-xl border border-slate-100 flex items-center justify-between group shadow-sm">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <div className="font-bold text-slate-800 text-sm flex items-center gap-2">
                                                            {p.sku}
                                                            {p.production_type === ProductionType.Imported && <span className="text-[8px] bg-purple-50 text-purple-600 px-1 rounded border border-purple-100 uppercase font-black">ΕΙΣ</span>}
                                                        </div>
                                                        <div className="text-[10px] text-slate-500 font-mono">
                                                            Κόστος: <span className="font-bold text-slate-700">{costToDisplay ? `${costToDisplay.toFixed(2)}€` : '-'}</span>
                                                        </div>
                                                    </div>
                                                </div>
                             ...
                             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {assignedMaterials.map(m => (
                                    <div key={m.id} className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex items-center gap-3">
                                        <div className="w-10 h-10 bg-purple-50 text-purple-600 rounded-lg flex items-center justify-center shrink-0"><Box size={18}/></div>
                                        <div><div className="font-bold text-slate-800 text-sm">{m.name}</div><div className="text-xs text-slate-500">{m.type} • {m.cost_per_unit}€ / {m.unit}</div></div>
                                    </div>
                                ))}
                             </div>
                        )}
                        ...
                        {activeTab === 'orders' && (
                            <div className="space-y-6">
                                <button onClick={() => setIsCreatingOrder(true)} className="w-full py-3 border-2 border-dashed border-slate-300 rounded-xl text-slate-50 font-bold bg-[#060b00] hover:bg-black transition-all flex items-center justify-center gap-2">
                                        ...
                                        <div key={o.id} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex justify-between items-center group hover:border-blue-200 transition-all">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="text-xs text-slate-500 font-medium">{new Date(o.created_at).toLocaleDateString('el-GR')} • {o.items.length} είδη</div>
                                                </div>
                                            </div>
                                            <div className="flex gap-2">
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-slate-400 p-10">
                    ...
                    <p className="font-bold">Επιλέξτε προμηθευτή από τη λίστα</p>
                </div>
            )}
            ...
            {isEditing && (
                <div className="absolute inset-0 bg-white z-50 flex flex-col animate-in slide-in-from-bottom-4">
                    ...
                    <div className="p-8 space-y-4 flex-1 overflow-y-auto max-w-2xl mx-auto w-full">
                        ...
                        <input value={supplierForm.contact_person || ''} onChange={e => setSupplierForm({...supplierForm, contact_person: e.target.value})} placeholder="Υπεύθυνος Επικοινωνίας" className="w-full p-4 border border-slate-200 rounded-xl outline-none"/>
                        <div className="grid grid-cols-2 gap-4">
                        ...
                        <button onClick={handleSaveSupplier} className="w-full py-4 bg-slate-900 text-white font-bold rounded-xl shadow-lg hover:bg-black transition-colors">Αποθήκευση Στοιχείων</button>
                    </div>
                </div>
            )}
                ...
                <div className="absolute inset-0 bg-white z-50 flex flex-col animate-in slide-in-from-right-4">
                    ...
                    <div className="flex-1 overflow-hidden flex flex-col lg:flex-row">
                        <div className="lg:w-1/3 p-6 border-r border-slate-100 overflow-y-auto space-y-6">
                            ...
                            <div className="relative">
                                <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16}/>
                                <input className="w-full pl-9 p-3 border border-slate-200 rounded-xl text-sm font-medium outline-none focus:ring-2 focus:ring-slate-200" placeholder={`Αναζήτηση ${poType === 'Material' ? 'υλικού' : 'κωδικού'}...`} value={poSearch} onChange={e => setPoSearch(e.target.value)} />
                                {poSearch && (
                                    <div className="absolute top-full left-0 right-0 bg-white border border-slate-100 rounded-xl shadow-xl mt-2 z-50 max-h-60 overflow-y-auto">
                                        {(poType === 'Material' ? materials : products)?.filter((i: any) => (i.name || i.sku).toLowerCase().includes(poSearch.toLowerCase())).slice(0, 10).map((item: any) => (
                                            <button key={item.id || item.sku} onClick={() => { handleAddToOrder(item, poType); setPoSearch(''); }} className="w-full text-left p-3 border-b border-slate-50 hover:bg-slate-50 flex justify-between items-center text-sm"><span className="font-bold text-slate-700">{item.name || item.sku}</span><Plus size={14} className="text-emerald-500"/></button>
                                        ))}
                                    </div>
                                )}
                            </div>
                            <textarea value={orderNotes} onChange={e => setOrderNotes(e.target.value)} placeholder="Σημειώσεις εντολής..." className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm h-24 resize-none outline-none"/>
                        </div>
                        <div className="flex-1 bg-slate-50/50 p-6 flex flex-col overflow-hidden">
                            <div className="flex-1 overflow-y-auto space-y-2 mb-4 pr-2 custom-scrollbar">
                                {orderItems.map((item, idx) => (
                                    <div key={idx} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between group animate-in slide-in-from-right-2">
                                        <div className="flex-1"><div className="font-black text-slate-800 text-sm">{item.item_name}</div><div className="text-[10px] text-slate-400 font-bold uppercase tracking-wide">{item.item_type === 'Product' ? 'Προϊον' : 'Υλικο'}</div></div>
                                        <div className="flex items-center gap-6">
                                            <div className="flex items-center gap-2"><span className="text-[10px] font-bold text-slate-400 uppercase">Κοστος</span><input type="number" value={item.unit_cost} onChange={e => updateOrderItem(idx, 'cost', parseFloat(e.target.value)||0)} className="w-20 p-1.5 border rounded-lg text-right text-sm font-mono font-bold bg-slate-50"/></div>
                                            <div className="flex items-center gap-2"><span className="text-[10px] font-bold text-slate-400 uppercase">Ποσ.</span><input type="number" value={item.quantity} onChange={e => updateOrderItem(idx, 'qty', parseInt(e.target.value)||1)} className="w-16 p-1.5 border rounded-lg text-center text-sm font-bold bg-slate-50"/></div>
                                            <div className="font-black text-slate-900 w-24 text-right text-base">{formatCurrency(item.total_cost)}</div>
                                            <button onClick={() => setOrderItems(prev => prev.filter((_, i) => i !== idx))} className="text-slate-300 hover:text-red-600 transition-colors"><Trash2 size={18}/></button>
                                        </div>
                                    </div>
                                ))}
                            </div>
