...
import { EnrichedDeliveryItem, OrderDeliveryReminder, OrderShipment } from '../../../types';
      ...
      <div className="w-full max-h-[92vh] overflow-y-auto bg-slate-50 rounded-t-[2.5rem] p-4 pb-safe animate-in slide-in-from-bottom-full duration-300">
        <div className="flex justify-end mb-3">
          <button
            ...
            className="w-10 h-10 rounded-full bg-white border border-slate-200 text-slate-500 flex items-center justify-center"
          >
            <X size={18} />
          </button>
        </div>
        <DeliveryDetailPanel
          ...
          onRevertShipment={props.onRevertShipment}
          loadingReminders={props.loadingReminders}
        />
      </div>
    </div>
  );
