...
import { Order, OrderStatus, Product, ProductVariant, OrderItem, Customer, VatRegime, Material, MaterialType } from '../types';
import { ArrowLeft, Save, Plus, Search, Phone, User, Check, X, ChevronRight, Hash, Coins, Percent, AlertCircle, ImageIcon, Box, Camera, StickyNote, Minus, Trash2, Layers } from 'lucide-react';
import { useQuery, useQueryClient } from '@tanstack/react-query';
import { api, SYSTEM_IDS } from '../lib/supabase';
import { useUI } from './UIProvider';
import { formatCurrency, getVariantComponents, findProductByScannedCode, splitSkuComponents } from '../utils/pricingEngine';
...
export default function OrderBuilder({ onBack, initialOrder, products, materials }: Props) {
    ...
    }, [initialOrder]); // Run once on mount if new order
                ...
                <div className="lg:col-span-3 space-y-6 overflow-y-auto pr-2 custom-scrollbar border-r border-slate-50">
                    ...
                    <div className="bg-emerald-50 p-6 rounded-2xl border border-emerald-200 shadow-sm sticky bottom-0 space-y-2">
                        ...
                        <button onClick={handleSaveOrder} className="w-full bg-[#060b00] text-white py-3.5 rounded-xl font-bold hover:bg-black transition-all shadow-lg active:scale-95 flex items-center justify-center gap-2">
                            <Save size={18}/> {initialOrder ? 'Ενημέρωση' : 'Καταχώρηση'}
                        </button>
                    </div>
                </div>
