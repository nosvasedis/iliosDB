...
import { getProductionStageLabel, PRODUCTION_STAGES } from '../../utils/productionStages';
        ...
        <div className="mt-2 pt-2 border-t border-slate-200/50">
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        disabled={isMoving}
                        className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-bold transition-colors shadow-sm active:scale-95 ${
                            isMoving
                    ...
                    >
                        {batch.on_hold ? (
                            <PlayCircle size={12} className="fill-current" />
                        ) : (
                            <PauseCircle size={12} />
                        )}
            ...
            {isOpen &&
                ReactDOM.createPortal(
                    ...
                    >
                        <div className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-2 px-2 sticky top-0 bg-white pt-1">
                            Επιλογή Σταδίου
                        </div>
                        <div className="space-y-1">
                            {FINDER_STAGE_ORDER.map((stage, index) => {
                                ...
                                const colorKey =
                                    stage.id === ProductionStage.AwaitingDelivery
                                        ? 'AwaitingDelivery'
                                        : stage.id === ProductionStage.Waxing
                                          ? 'Waxing'
                                          : stage.id === ProductionStage.Casting
                                            ? 'Casting'
                                            : stage.id === ProductionStage.Setting
                                              ? 'Setting'
                                              : stage.id === ProductionStage.Polishing
                                                ? 'Polishing'
                                                : stage.id === ProductionStage.Assembly
                                                  ? 'Assembly'
                                ...
                                if (stage.id === ProductionStage.Polishing) {
                                    ...
                                    return (
                                        <div key={stage.id} className="flex gap-1">
                                            <button
                                                onClick={() =>
                                                    handleStageSelect(ProductionStage.Polishing, {
                                                        pendingDispatch: true,
                                                    })
                                                }
                                                disabled={isDisabled}
                                                className={`flex-1 text-center px-2 py-1.5 rounded-lg text-[10px] font-bold transition-colors flex items-center justify-between
                                                ${
                                                    isCurrentPending
                                ...
                                return (
                                    <button
                                        ...
                                        disabled={isDisabled}
                                        className={`w-full text-left px-2.5 py-1.5 rounded-lg text-[11px] font-bold transition-colors flex items-center justify-between
                                        ${
                                            isCurrent
