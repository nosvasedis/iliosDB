...
async function fetchFullTable(
    ...
    options?: { bypassInspectionCheck?: boolean },
): Promise<any[]> {
    if (!options?.bypassInspectionCheck) {
        assertInspectionTableAllowed(tableName);
    }
...
export const api = {
    ...
    deleteLegalDocument: async (documentId: string, userName?: string | null): Promise<void> => {
        ...
        if (document.aa || !['draft', 'failed'].includes(document.status)) throw new Error('Το παραστατικό διατηρείται στο αρχείο και δεν διαγράφεται.');
    ...
    saveLegalDraft: async (document: LegalDocument, lines: LegalDocumentLine[]): Promise<void> => {
        if (isLocalMode || !navigator.onLine) throw new Error('Η αποθήκευση παραστατικού απαιτεί σύνδεση.');
        if (!['draft', 'failed'].includes(document.status)) throw new Error('Το παραστατικό είναι κλειδωμένο.');
        const settings = await api.getLegalSettings();
        assertInspectionRpcAllowed('save_sbz_draft');
        const { error } = await supabase.rpc('save_sbz_draft', {
            p_document: serializeLegalDocumentForDb({ ...document, environment: document.environment || settings.environment }),
            p_lines: lines.map(line => serializeLegalDocumentLineForDb(line, document.id)),
        });
        if (error) throw new Error(error.message);
    },

    syncTransmittedLegalDocuments: async (params: LegalSyncParams): Promise<LegalSyncRun> => api.callSbz('/sbz/sync', params),

    submitLegalDocument: async (documentId: string, userName?: string | null): Promise<LegalDocument> => api.callSbz('/sbz/submit', { documentId }),

    cancelLegalDocument: async (documentId: string, userName?: string | null): Promise<LegalDocument> => api.callSbz('/sbz/cancel-delivery', { documentId }),

    reconcileLegalDocument: async (documentId: string): Promise<LegalDocument> => api.callSbz('/sbz/reconcile', { documentId }),
    ...
    sendLegalDeliveryEvent: async (documentId: string, eventType: LegalDeliveryEvent['event_type'], route: string, userName?: string | null, extraPayload?: Record<string, unknown>): Promise<void> => {
        throw new Error('Η ενημέρωση διακίνησης αναμένει υποστήριξη από την SBZ. Δεν πραγματοποιείται απευθείας διαβίβαση.');
    ...
    callSbz: async (route: string, payload?: unknown): Promise<any> => {
        if (isLocalMode || !navigator.onLine) throw new Error('Η ενέργεια απαιτεί σύνδεση στο διαδίκτυο.');
        assertInspectionWorkerRouteAllowed(route);
        const { data: { session } } = await supabase.auth.getSession();
        if (!session) throw new Error('Συνδεθείτε ξανά για να συνεχίσετε.');
        const response = await fetch(CLOUDFLARE_WORKER_URL + route, {
            method: payload === undefined ? 'GET' : 'POST',
            headers: { Authorization: 'Bearer ' + session.access_token, ...(payload instanceof FormData ? {} : { 'Content-Type': 'application/json' }) },
            body: payload === undefined ? undefined : payload instanceof FormData ? payload : JSON.stringify(payload),
        });
        const result = await response.json();
        if (!response.ok) throw new Error(result.error || 'Η ενέργεια δεν ολοκληρώθηκε.');
        return result;
    },
    ...
    getBackupTableStats: async (): Promise<Record<string, number>> => {
        const stats: Record<string, number> = {};
        for (const entry of BACKUP_TABLE_REGISTRY) {
            try {
                if (isLocalMode || !navigator.onLine) {
                    const data = await offlineDb.getTable(entry.table);
                    stats[entry.table] = data?.length ?? 0;
                } else {
                    const { count, error } = await supabase
                        .from(entry.table)
                        .select('*', { count: 'exact', head: true });
                    stats[entry.table] = error ? 0 : (count ?? 0);
                }
            } catch {
                stats[entry.table] = 0;
            }
    ...
    getSystemExport: async (exportOptions?: BackupExportOptions, onProgress?: ProgressCallback): Promise<BackupEnvelope> => {
        ...
        for (let i = 0; i < totalTables; i++) {
            ...
            onProgress?.({
                ...
                total: totalTables,
                tableName,
                message: `Εξαγωγή ${entry?.displayName ?? tableName} (${i + 1}/${totalTables})...`,
            });
            try {
        ...
        if (includeImages) {
            const imgResult = await exportProductImages(tableData['products'] || [], R2_PUBLIC_URL, onProgress);
            images = imgResult.images;
            failedImages = imgResult.failedImages;
        }

        if (options.includeConfig) {
            onProgress?.({ phase: 'config', current: 1, total: 1, message: 'Αποθήκευση ρυθμίσεων σύνδεσης...' });
        }
        const config = readConfigForExport(options);

        let syncQueue: any[] = [];
        if (options.includeSyncQueue) {
            onProgress?.({ phase: 'sync_queue', current: 1, total: 1, message: 'Αποθήκευση ουράς συγχρονισμού...' });
            syncQueue = await offlineDb.getQueue();
        }

        let extras: Record<string, unknown> = {};
        if (options.includeLocalExtras) {
            onProgress?.({ phase: 'extras', current: 1, total: 1, message: 'Αποθήκευση τοπικών ρυθμίσεων...' });
    ...
    getFullSystemExport: async (onProgress?: ProgressCallback): Promise<BackupEnvelope> => {
        return api.getSystemExport(getDefaultExportOptions(), onProgress);
    },

    restoreSystem: async (
        backupData: BackupEnvelope | Record<string, any[]>,
        ...
        if (isLocalMode || !SUPABASE_URL) {
            ...
            if (includeImages && envelope?._images) {
                await patchLocalProductImages(
                    envelope._images,
                    async () => (await offlineDb.getTable('products')) || [],
                    async (products) => { await offlineDb.saveTable('products', products); },
                );
            }
            ...
            if (options.includeSyncQueue && envelope?._sync_queue?.length) {
            ...
            if (options.includeLocalExtras && envelope?._extras) {
                onProgress?.({ phase: 'extras', current: 1, total: 1, message: 'Επαναφορά τοπικών ρυθμίσεων...' });
        ...
        if (includeImages && envelope?._images) {
            ...
            imageFailures.push(...imgResult.failures);
            for (const f of imgResult.failures) {
                errors.push({ table: '_images', message: `Αποτυχία εικόνας: ${f}` });
            }
        }
    ...
    restoreFullSystem: async (
        ...
        const isEnvelope = !!(backupData as BackupEnvelope)._meta;
        const envelope = isEnvelope ? (backupData as BackupEnvelope) : null;
        const restoreOpts: BackupRestoreOptions = {
            tables: options?.tables ?? getDefaultRestoreOptions().tables,
            includeImages: options?.includeImages ?? true,
            restoreConfig: options?.restoreConfig ?? false,
            includeSyncQueue: options?.includeSyncQueue ?? !!(envelope?._sync_queue?.length),
            includeLocalExtras: options?.includeLocalExtras ?? true,
            onProgress: options?.onProgress,
        };
        return api.restoreSystem(backupData, restoreOpts);
    },
